...
type Mode = "home" | "today" | "dream" | "random";
...
const STORAGE_KEY = "haemala_growth_history_v1";

const MODE_TEXT = {
  home: {
    badge: "HAEMALA GROWTH EDITION",
    title: "꿈을 번호로 번역하다",
    subtitle: "지금 이 순간, 당신의 꿈은 어떤 로또 번호로 번역될까?",
    description:
      "꿈 해몽, 보안 난수, 번호 분산, 홀짝 균형, 합계 안정성, 구간 편향 회피를 결합한 무료 로또 번호 추천 서비스입니다.",
    inputTitle: "꿈 내용 입력",
    button: "프리미엄 번호 리포트 생성",
    placeholder: "예: 낯선 바닷가에서 금빛 문을 열었고, 안쪽에 돈과 밝은 빛이 보였어요.",
  },
  today: {
    badge: "TODAY LOTTO READING",
    title: "오늘의 로또 번호",
    subtitle: "오늘 하루의 기운을 번호 조합으로 확인하세요.",
    description:
      "오늘의 로또 번호 추천, 무료 로또 번호 생성기, 운세형 번호 리포트를 한 번에 확인할 수 있습니다.",
    inputTitle: "오늘의 기분 또는 상황 입력",
    button: "오늘의 번호 생성",
    placeholder: "예: 오늘 이상하게 기분이 좋고, 뭔가 시작해보고 싶은 느낌이에요.",
  },
  dream: {
    badge: "DREAM LOTTO READING",
    title: "꿈 해몽 로또 번호",
    subtitle: "꿈에 나온 상징을 번호 리포트로 변환합니다.",
    description:
      "물, 돈, 불, 길, 집, 바다, 동물, 빛 같은 꿈 상징을 기반으로 무료 로또 번호를 추천합니다.",
    inputTitle: "꿈 내용 자세히 입력",
    button: "꿈 해몽 번호 생성",
    placeholder: "예: 꿈에서 큰 바다를 건너 금빛 집에 들어갔고, 하얀 동물이 저를 바라봤어요.",
  },
  random: {
    badge: "RANDOM NUMBER GENERATOR",
    title: "무료 로또 번호 생성기",
    subtitle: "복잡한 입력 없이 바로 번호를 생성하세요.",
    description:
      "완전 랜덤에 가까운 번호 추천이지만, 합계·홀짝·구간 편향을 보정해 보기 좋은 조합으로 제공합니다.",
    inputTitle: "선택 입력",
    button: "랜덤 번호 생성",
    placeholder: "입력하지 않아도 됩니다. 원하면 오늘의 느낌을 짧게 적어도 됩니다.",
  },
};
...
function hasWeakPattern(nums: number[]) {
  ...
  const zones = sorted.reduce<Record<number, number>>((acc, n) => {
...
function weightedNumber(seed: number, dream: string, used: Set<number>) {
  const signal = [...dream].reduce((acc, ch, idx) => acc + ch.charCodeAt(0) * (idx + 11), seed);
...
function createPick(input: string, index: number): PickSet {
  const seed = hashText(`${input}-${Date.now()}-${cryptoRand(999999)}-${index}`);
  const used = new Set<number>();

  let tries = 0;
  while (used.size < 6 && tries < 300) {
    used.add(weightedNumber(seed + tries * 193, input, used));
    tries++;
  }
  ...
  while (hasWeakPattern(numbers) && repair < 100) {
    ...
    while (temp.size < 6) {
      temp.add(weightedNumber(seed + cryptoRand(999999), input, temp));
    }
...
function getSignals(text: string) {
  const found = DREAM_TAGS.filter((x) => text.includes(x.key));
...
export default function HaemalaApp({ mode = "home" }: { mode?: Mode }) {
  const copy = MODE_TEXT[mode];
  const [dream, setDream] = useState(mode === "random" ? "완전 랜덤 번호 생성" : "");
  ...
  const generate = () => {
    const input = mode === "random" && !dream.trim() ? `랜덤-${Date.now()}` : dream.trim();
    if (!input) return;

    setLoading(true);

    setTimeout(() => {
      const nextPicks = Array.from({ length: 5 }, (_, i) => createPick(input, i));
      setPicks(nextPicks);
  ...
  const shareService = async () => {
    ...
    const text = "무료 로또 번호 추천 서비스 해말아에서 오늘의 번호 리포트를 뽑아봤어요.";

    try {
      if (navigator.share) {
        await navigator.share({ title: "해말아 - 무료 로또 번호 추천", text, url });
      } else {
    ...
    <main className="min-h-screen overflow-hidden bg-[#07070a] text-white">
      ...
      <section className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 px-5 py-8 sm:px-8 lg:py-12">
          ...
          <nav className="mb-7 flex flex-wrap items-center gap-3">
            {[
              ...
              <a
                key={href}
                href={href}
                className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs font-black text-zinc-300 hover:border-amber-300/40 hover:text-amber-200"
              >
                {label}
              </a>
            ))}
        ...
        <section id="input" className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl backdrop-blur-xl sm:p-7">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm font-black text-amber-200">INPUT</p>
            ...
            <button
              onClick={generate}
              disabled={loading || (mode !== "random" && !dream.trim())}
            ...
            >
              {loading ? "번호 리포트 생성 중..." : copy.button}
            </button>
          </div>

          <aside className="grid gap-5">
            <div className="rounded-[2rem] border border-amber-300/15 bg-black/35 p-5 shadow-2xl backdrop-blur-xl sm:p-7">
              <p className="text-sm font-black text-amber-200">GROWTH ENGINE</p>
              <h2 className="mt-1 text-2xl font-black">수익화를 위한 구조</h2>

              <div className="mt-6 space-y-4">
                {[
                  ["SEO 페이지", "오늘의 로또 번호, 꿈 해몽 로또, 무료 로또 번호 생성기 페이지로 검색 유입을 넓힙니다."],
                  ["공유 퍼널", "번호 복사, 이미지 저장, 링크 공유로 카톡·SNS 유입을 만듭니다."],
                  ["재방문 히스토리", "DB 없이 브라우저에 최근 결과를 저장해 다시 보게 만듭니다."],
                  ["광고 슬롯", "승인 후 상단·입력부·결과부·하단에 광고를 배치할 수 있습니다."],
                ].map(([a, b]) => (
        ...
        {picks.length > 0 && (
          <section id="result" className="grid gap-6 lg:grid-cols-[1fr_320px]">
            ...
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <h2 className="mt-1 text-3xl font-black sm:text-4xl">오늘의 번호 리포트</h2>
                  <p className="mt-2 text-sm font-semibold text-zinc-400">
                    무료 로또 번호 추천 5조합 · 보너스 후보 포함
                  </p>
                </div>

                <div className="flex flex-wrap gap-2">
                  <button onClick={saveImage} className="rounded-2xl bg-amber-300 px-5 py-3 text-sm font-black text-black">
                    이미지 저장
                  </button>
                  <button onClick={copyNumbers} className="rounded-2xl border border-white/15 bg-white/[0.06] px-5 py-3 text-sm font-black text-white">
                    번호 복사
                  </button>
                  <button onClick={shareService} className="rounded-2xl border border-white/15 bg-white/[0.06] px-5 py-3 text-sm font-black text-white">
                    공유
                  </button>
                </div>
              </div>
              ...
              <div className="mt-7 grid gap-5">
                {picks.map((pick, idx) => (
                  <div key={pick.id}>
                    {idx === 2 && <div className="mb-5"><AdSlot label="IN RESULT AD" /></div>}

                    <article className="overflow-hidden rounded-[1.8rem] border border-white/10 bg-white/[0.055]">
                      ...
                      <div className="grid gap-4 p-5 lg:grid-cols-[0.8fr_1.2fr]">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          {pick.reason.map((r) => (
                            <p key={r} className="rounded-2xl border border-white/10 bg-black/25 p-4 text-sm font-semibold leading-6 text-zinc-300">
                              {r}
                            </p>
                          ))}
                        </div>
            ...
            <aside className="grid content-start gap-5">
              <AdSlot label="RESULT SIDE AD" />

              <div className="rounded-[1.5rem] border border-amber-300/20 bg-amber-300/10 p-5">
                <p className="text-lg font-black text-amber-100">친구도 번호 뽑게 하기</p>
                <p className="mt-2 text-sm font-semibold leading-6 text-zinc-300">
                  결과 이미지를 저장해서 카톡, 인스타, 커뮤니티에 공유하면 다른 사람도 바로 번호를 뽑아볼 수 있습니다.
                </p>
                <button onClick={shareService} className="mt-4 w-full rounded-2xl bg-amber-300 px-4 py-3 text-sm font-black text-black">
                  링크 공유하기
                </button>
              </div>

              <button onClick={generate} className="rounded-[1.5rem] border border-white/10 bg-white/[0.06] p-5 text-left font-black text-white">
                같은 조건으로 다시 뽑기
                <span className="mt-2 block text-sm font-semibold text-zinc-400">
                  같은 입력도 매번 다른 보안 난수를 반영합니다.
                </span>
              </button>
            </aside>
          </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_320px]">
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-5 shadow-2xl backdrop-blur-xl sm:p-7">
            <div className="flex items-center justify-between gap-4">
              ...
              {history.length > 0 && (
                <button onClick={clearHistory} className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs font-black text-zinc-400">
                  삭제
                </button>
              )}
            </div>
            ...
            {history.length === 0 ? (
              ...
              <div className="mt-5 grid gap-3">
                {history.map((item) => (
                  <button key={item.id} onClick={() => loadHistory(item)} className="rounded-2xl border border-white/10 bg-black/25 p-4 text-left hover:border-amber-300/40">
                    <p className="truncate text-sm font-black text-white">{item.dream}</p>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-6 shadow-2xl backdrop-blur-xl sm:p-8">
          ...
          <h2 className="mt-1 text-2xl font-black">무료 로또 번호 추천 FAQ</h2>

          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {[
              [
                "무료 로또 번호 추천이 실제로 맞나요?",
                "복권은 확률 게임이므로 당첨을 보장할 수 없습니다. 해말아는 오락용 번호 리포트를 제공합니다.",
              ],
              [
                "꿈 해몽 로또 번호는 어떻게 생성되나요?",
                "입력한 꿈의 상징, 보안 난수, 번호 분산, 합계, 홀짝 균형을 함께 반영합니다.",
              ],
              [
                "개인정보가 저장되나요?",
        ...
        <footer className="pb-6 text-center text-xs font-bold leading-6 text-zinc-500">
          HAEMALA · 무료 로또 번호 추천 · 꿈 해몽 로또 번호 · 오늘의 로또 번호 · 무료 로또 번호 생성기
          <br />
